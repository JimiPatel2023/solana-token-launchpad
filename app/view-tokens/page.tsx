...
export default function MyTokens() {
	...
	return (
		<motion.main className="container mx-auto px-4 py-16 relative z-10" initial="hidden" animate="visible" variants={containerVariants}>
			<motion.h1 className="text-5xl font-bold text-center mb-12 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}>
				My Created Tokens
			</motion.h1>
			{!publicKey ? (
				<motion.div className="bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-2xl p-8 text-center shadow-lg backdrop-blur-md" variants={itemVariants}>
					<p className="text-xl text-gray-300 mb-4">Please connect your wallet to view your created tokens.</p>
					<button className="bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold py-3 px-6 rounded-full hover:from-purple-600 hover:to-pink-600 transition-colors duration-300 shadow-md hover:shadow-lg">Connect Wallet</button>
				</motion.div>
			) : isLoading ? (
				...
				<motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" variants={containerVariants}>
					{tokens.map((token, index) => (
						<motion.div key={index} variants={itemVariants}>
							<TokenCard title={token.name} symbol={token.symbol} address={token.address} icon={token.image || "/default-token-icon.png"} href={`/token-details/${token.address}`} onCopy={copyToClipboard} />
						</motion.div>
					))}
				</motion.div>
			) : (
				<motion.div className="bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-2xl p-8 text-center shadow-lg backdrop-blur-md" variants={itemVariants}>
					...
					<Link href="/create-token" className="bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold py-3 px-6 rounded-full hover:from-purple-600 hover:to-pink-600 transition-colors duration-300 shadow-md hover:shadow-lg">
